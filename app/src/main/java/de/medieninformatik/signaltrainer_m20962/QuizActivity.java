    ...
    private void initQuestion(){
       ...
       new DownloadImageTask(quizImgView).execute(signal.url);
        ...
        for (RadioButton rbs: rbArray) {
            rbs.setText(signallist.get(rnd.nextInt(signallist.size())).name);//setze den Text eines der RadioButtons anhand eines zufällig gewählten Signals
        }
        rbArray[antwort].setText(signal.name);//setze die Antwort zufällig
    }
